<script lang="ts">
	...
	const rows = $derived(data ? data.settings.panes.cardClassDetail : []);
</script>

<svelte:head><title>{cardClass ? m.cards_class_title({ cardClass: stringUtils.cardClass(cardClass) }) : m.cards_class_title_fallback()}</title></svelte:head>

{#if data}
	{#if data.settings.campaignCategoriesOnly}
		<CategoryHiddenNotice />
	{:else if !cardClass}
		<BorderedContainer><div class="p-8"><HelpParagraph>{m.cards_class_unknown()}</HelpParagraph></div></BorderedContainer>
	{:else}

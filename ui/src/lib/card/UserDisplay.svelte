...
<script lang="ts">
	...
	const { username, card, size = 'md' }: Props = $props();
	...
	const circleWrapClass = $derived(
		size === 'lg' ? 'scale-[2] origin-center mx-4' : '',
	);
</script>
...
<div class="flex items-center {gapClass}">
	...
	{:else}
		...
		></div>
	{/if}
	<span class="text-primary-900 dark:text-primary-100 {textClass}">{username}</span>
</div>

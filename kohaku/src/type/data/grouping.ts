...
function groupAssets<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  const filtered = items.filter((wrapper) => {
    ...
    return (
      card.cardType === CardType.Asset && card.weakness === undefined && card.permanent !== true
    );
  });
...
function groupBySlot<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  ...
  if (allySlot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotAlly,
      dynamicGrouping: false,
      items: allySlot.map((w) => w.item),
    });

  const accessorySlot = items.filter(
    (w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.Accessory
  );
  if (accessorySlot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotAccessory,
      dynamicGrouping: false,
      items: accessorySlot.map((w) => w.item),
    });

  const arcaneslot = items.filter(
    (w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.Arcane
  );
  if (arcaneslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotArcane,
      dynamicGrouping: false,
      items: arcaneslot.map((w) => w.item),
    });

  const twoArcaneslot = items.filter(
    (w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.ArcaneX2
  );
  if (twoArcaneslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotArcaneX2,
      dynamicGrouping: false,
      items: twoArcaneslot.map((w) => w.item),
    });

  const bodyslot = items.filter((w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.Body);
  if (bodyslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotBody,
      dynamicGrouping: false,
      items: bodyslot.map((w) => w.item),
    });

  const handslot = items.filter((w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.Hand);
  if (handslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotHand,
      dynamicGrouping: false,
      items: handslot.map((w) => w.item),
    });

  const twoHandslot = items.filter(
    (w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.HandX2
  );
  if (twoHandslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotHandX2,
      dynamicGrouping: false,
      items: twoHandslot.map((w) => w.item),
    });

  const tarotslot = items.filter(
    (w) => w.card.slots?.length === 1 && w.card.slots[0] === Slot.Tarot
  );
  if (tarotslot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotTarot,
      dynamicGrouping: false,
      items: tarotslot.map((w) => w.item),
    });

  const noSlot = items.filter((w) => w.card.slots === undefined || w.card.slots.length === 0);
  if (noSlot.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotNone,
      dynamicGrouping: false,
      items: noSlot.map((w) => w.item),
    });

  const multipleSlots = items.filter((w) => w.card.slots !== undefined && w.card.slots.length > 1);
  if (multipleSlots.length > 0)
    groups.push({
      groupingKey: GroupingKey.SlotMultiple,
      dynamicGrouping: false,
      items: multipleSlots.map((w) => w.item),
    });
...
function groupByCost<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  ...
  if (cost0.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost0,
      dynamicGrouping: false,
      items: cost0.map((w) => w.item),
    });

  const cost1 = items.filter((w) => w.card.cost === 1);
  if (cost1.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost1,
      dynamicGrouping: false,
      items: cost1.map((w) => w.item),
    });

  const cost2 = items.filter((w) => w.card.cost === 2);
  if (cost2.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost2,
      dynamicGrouping: false,
      items: cost2.map((w) => w.item),
    });

  const cost3 = items.filter((w) => w.card.cost === 3);
  if (cost3.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost3,
      dynamicGrouping: false,
      items: cost3.map((w) => w.item),
    });

  const cost4 = items.filter((w) => w.card.cost === 4);
  if (cost4.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost4,
      dynamicGrouping: false,
      items: cost4.map((w) => w.item),
    });

  const cost5 = items.filter((w) => w.card.cost === 5);
  if (cost5.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost5,
      dynamicGrouping: false,
      items: cost5.map((w) => w.item),
    });

  const cost6Plus = items.filter((w) => typeof w.card.cost === 'number' && w.card.cost >= 6);
  if (cost6Plus.length > 0)
    groups.push({
      groupingKey: GroupingKey.Cost6Plus,
      dynamicGrouping: false,
      items: cost6Plus.map((w) => w.item),
    });

  const costX = items.filter((w) => w.card.cost === 'x');
  if (costX.length > 0)
    groups.push({
      groupingKey: GroupingKey.CostX,
      dynamicGrouping: false,
      items: costX.map((w) => w.item),
    });

  const noCost = items.filter((w) => w.card.cost === undefined);
  if (noCost.length > 0)
    groups.push({
      groupingKey: GroupingKey.CostNone,
      dynamicGrouping: false,
      items: noCost.map((w) => w.item),
    });
...
function groupByLevel<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  ...
  if (noLevel.length > 0)
    groups.push({
      groupingKey: GroupingKey.LevelNone,
      dynamicGrouping: false,
      items: noLevel.map((w) => w.item),
    });

  const level0 = items.filter((w) => w.card.xp === 0);
  if (level0.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level0,
      dynamicGrouping: false,
      items: level0.map((w) => w.item),
    });

  const level1 = items.filter((w) => w.card.xp === 1);
  if (level1.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level1,
      dynamicGrouping: false,
      items: level1.map((w) => w.item),
    });

  const level2 = items.filter((w) => w.card.xp === 2);
  if (level2.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level2,
      dynamicGrouping: false,
      items: level2.map((w) => w.item),
    });

  const level3 = items.filter((w) => w.card.xp === 3);
  if (level3.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level3,
      dynamicGrouping: false,
      items: level3.map((w) => w.item),
    });

  const level4 = items.filter((w) => w.card.xp === 4);
  if (level4.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level4,
      dynamicGrouping: false,
      items: level4.map((w) => w.item),
    });

  const level5 = items.filter((w) => w.card.xp === 5);
  if (level5.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level5,
      dynamicGrouping: false,
      items: level5.map((w) => w.item),
    });
...
function groupByLevelGrouped<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  ...
  const level1to5 = items.filter(
    (w) => typeof w.card.xp === 'number' && w.card.xp >= 1 && w.card.xp <= 5
  );
  if (level1to5.length > 0)
    groups.push({
      groupingKey: GroupingKey.Level1To5,
      dynamicGrouping: false,
      items: level1to5.map((w) => w.item),
    });
...
function groupByCommitPower<T>(items: CardItemWrapper<T>[]): GroupedResult<T>[] {
  ...
  if (power0.length > 0)
    groups.push({
      groupingKey: GroupingKey.CommitPower0,
      dynamicGrouping: false,
      items: power0.map((w) => w.item),
    });

  const power1 = items.filter((w) => getCommitPower(w.card) === 1);
  if (power1.length > 0)
    groups.push({
      groupingKey: GroupingKey.CommitPower1,
      dynamicGrouping: false,
      items: power1.map((w) => w.item),
    });

  const power2 = items.filter((w) => getCommitPower(w.card) === 2);
  if (power2.length > 0)
    groups.push({
      groupingKey: GroupingKey.CommitPower2,
      dynamicGrouping: false,
      items: power2.map((w) => w.item),
    });

  const power3 = items.filter((w) => getCommitPower(w.card) === 3);
  if (power3.length > 0)
    groups.push({
      groupingKey: GroupingKey.CommitPower3,
      dynamicGrouping: false,
      items: power3.map((w) => w.item),
    });

  const power4Plus = items.filter((w) => getCommitPower(w.card) >= 4);
  if (power4Plus.length > 0)
    groups.push({
      groupingKey: GroupingKey.CommitPower4Plus,
      dynamicGrouping: false,
      items: power4Plus.map((w) => w.item),
    });
